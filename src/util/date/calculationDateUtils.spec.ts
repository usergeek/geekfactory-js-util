...
describe("DateTimeUtils", () => {
    it("getHMSFromMillis", () => {
        ...
        expect(fn("2022-12-28T00:00:00", "2022-12-29T06:54:32")).toStrictEqual({milliseconds: 0, seconds: 32, minutes: 54, hours: 6, days: 1})

        const now = new Date().getTime()
        expect(getHMSFromMillis(now - now)).toStrictEqual({milliseconds: 0, seconds: 0, minutes: 0, hours: 0, days: 0})
        expect(getHMSFromMillis(123)).toStrictEqual({milliseconds: 123, seconds: 0, minutes: 0, hours: 0, days: 0})
        expect(getHMSFromMillis(325 + new Date("2022-12-29T06:54:32").getTime() - new Date("2022-12-28T00:00:00").getTime())).toStrictEqual({milliseconds: 325, seconds: 32, minutes: 54, hours: 6, days: 1})
    })
